...
def getNameEmojiMessage():
    ...
    emoji_list = list()
    for n_chair_index in range(len(name)):
        emoji_list.append({
            "index":n_chair_index,
            "emojiId":f"{lookUpStr.index(name[n_chair_index])+1:03d}",#固定要塞0/前面要三位(三個字串‘)/
            "productId":productId
        })
...
def getTaipei101LocationMessage():
    ...
    message["latitude"]: 24.6007925
    message["longitude"]: 120.5696926
    return message


def get101videoMessage():
    message = dict()
    message["type"] ="video"
...
def line_login():
    if request.method == 'GET':
        ...
        if code and state:
            ...
            statusMessage = content.get("statusMessage","") #get or default
            print(content)
